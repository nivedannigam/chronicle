import type { RawMetricRow } from '@/features/health/extraction/metric-extraction.engine'
...
function pushRow(
	...
	const name = stripTechnologySuffix(normalizeLine(rawName))
	...
	if (/^(?:Tests Done|HDFC COMBO|COMPLETE URINE|LIPID PROFILE)/i.test(name)) {
		return
	}
...
function parseUrineVertical(
	...
	while (index + 3 < lines.length) {
		...
		pushRow(rows, name, value, referenceRange, unit === '-' ? null : unit, 0.9)
		index += 4
...
function parseCbcVertical(
	...
	while (index + 3 < lines.length) {
		...
		pushRow(rows, rawName, value, referenceRange, unit, 0.93)
		index += 4
...
function parseGoogleOcrBlocks(lines: string[]): RawMetricRow[] {
	const rows: RawMetricRow[] = []
	let index = 0
	...
	while (index < lines.length) {
		...
		if (line === 'NEGATIVE') {
			...
			if (/COTININE/i.test(next)) {
				pushRow(rows, 'COTININE', 'NEGATIVE', '', null)
				index += 2
				continue
			}
		}
		...
		if (line === 'NON REACTIVE' && /HIV/i.test(lines[index + 1] ?? '')) {
			pushRow(rows, 'HIV I and II', 'NON REACTIVE', '< 1', 'OD ratio')
			index += 2
			continue
		}
...
function dedupeRows(rows: RawMetricRow[]): RawMetricRow[] {
	const deduped = new Map<string, RawMetricRow>()
...
export function extractThyrocareMetricsFromText(text: string): RawMetricRow[] {
	...
	return dedupeRows(parseGoogleOcrBlocks(lines))
}
