...
import type { VehicleDiscoveryRunSummary } from '@/features/vehicle-import/types/vehicle-import.types'
...
async function completeDiscoveryRun(
	runId: string,
	summary: Partial<VehicleDiscoveryRunSummary> & {
		status: VehicleDiscoveryRunSummary['status']
	},
) {
...
export async function runVehicleDiscovery(input: {
	userId: string
	modifiedSince?: string | null
}): Promise<{
	run: VehicleDiscoveryRunSummary
	discoveredCount: number
}> {
	const assignments = await listVehicleSourceAssignments(input.userId)
	const folderIds = [...new Set(assignments.map((a) => a.externalFolderId))]

	if (folderIds.length === 0) {
		throw new Error(
			'No vehicle folders configured. Assign a Vehicles folder in Vehicle Settings first.',
		)
	}
	...
	try {
		...
		for (const item of response.items) {
			...
			const assignment = resolveVehicleAssignmentForFile(item, assignments)
			...
			if (
				existing?.targetModule === 'vehicles' &&
			...
			await upsertRegistryRecord({
				...
				discoveryCategory: 'vehicle_document',
				discoveryConfidence: 0.85,
				discoveryReason: 'Vehicle folder PDF',
		...
		await completeDiscoveryRun(run.id, {
			...
			duplicateCount: 0,
			errorMessage:
				error instanceof Error ? error.message : 'Vehicle discovery failed',
		})
