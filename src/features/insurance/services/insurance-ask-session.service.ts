...
const SESSION_INDEX_KEY = 'chronicle:insurance-ask:session-index'
const SESSION_DATA_PREFIX = 'chronicle:insurance-ask:session:'
...
export function loadInsuranceSessionTurns(
	sessionId: string,
): AskConversationTurn[] {
...
export function listInsuranceAskSessions(userId: string): AskSessionMeta[] {
	return readIndex()
...
export function resolveActiveInsuranceAskSession(input: {
	userId: string
	memberId: string | null
}): AskSessionMeta | null {
	const sessions = listInsuranceAskSessions(input.userId)
...
export function createInsuranceAskSession(input: {
	userId: string
	memberId: string | null
	...
	const session: AskSessionMeta = {
		...
		sessionKey: `insurance:${buildMemorySessionKey(input.userId, input.memberId)}`,
		createdAt: now,
		updatedAt: now,
		turnCount: 0,
		preview: '',
...
export function ensureInsuranceAskSession(input: {
	userId: string
	memberId: string | null
	memberName: string | null
}): AskSessionMeta {
	return (
		resolveActiveInsuranceAskSession({
			userId: input.userId,
			memberId: input.memberId,
		}) ?? createInsuranceAskSession(input)
	)
}

export function upsertInsuranceSessionFromTurns(input: {
	sessionId: string
	turns: AskConversationTurn[]
	memberId: string | null
	memberName: string | null
	...
	const updated: AskSessionMeta = existing
		...
		: {
				...
				memberName: input.memberName,
				sessionKey: userId
					? `insurance:${buildMemorySessionKey(userId, input.memberId)}`
					: null,
				createdAt: now,
				updatedAt: now,
				turnCount: input.turns.length,
...
export function deleteInsuranceAskSession(sessionId: string): void {
	if (typeof window === 'undefined') {
		return
	}
...
export function searchInsuranceAskSessions(
	userId: string,
	query: string,
): AskSessionMeta[] {
	...
	if (!normalized) {
		return listInsuranceAskSessions(userId)
	}

	return listInsuranceAskSessions(userId).filter((session) => {
		const turns = loadInsuranceSessionTurns(session.id)
