	...
	return {
		...
		semanticTimeline: pkg.semanticTimeline.map((group) => ({
			...
			events: group.events.map((event) => ({
				...
				date: event.date,
				label: event.label,
				kind: event.kind as import('@/features/semantic-memory/types/semantic-memory.types').TimelineEvent['kind'],
		...
		metricHistories: pkg.metricHistories.map((history) => ({
			...
			trend: history.trend,
			trendDirection:
				history.trendDirection as import('@/features/semantic-memory/types/semantic-memory.types').MetricHistoryRecord['trendDirection'],
...
function toRetrievedComparison(
	...
	return {
		...
		metrics: comparison.metrics.map((metric) => ({
			...
			status: metric.status as RetrievedComparison['metrics'][number]['status'],
		})),
	}
}
...
/** Maps legacy RetrievedKnowledge into the shared entity model (used by Health provider adapter). */
